...
function nameVerification() {
	var fullName =/^[a-zA-Z]+(?:[0-9]+)*$/;
	var fullNameInput = document.getElementById("name").value;
	  if (!fullName.test(fullNameInput)) {
	  	validateName = false;
	  	document.getElementById("nameText").innerHTML = "Invalid name";
	  }else {
	  	validateName = true;
...
function validateAddressSection() { 
  console.log(validateName, validateAddress);
  if (validateName && validateAddress) {
    document.getElementById("opacity").style = "opacity: 1;";
  } else {
    document.getElementById("opacity").innerHTML = "Something is missing";
  }
}